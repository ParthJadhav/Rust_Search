...
fn main() {
    ...
    match arg.as_str() {
        ...
        "sort" => {
            let home = dirs::home_dir().unwrap();
            let base: Vec<String> = SearchBuilder::default()
                .location(&home)
                .ext("rs")
                .build()
                .collect();
        ...
        "system" => {
            ...
            let (rs_count, _) =
                run_bench("system/ext_only (.rs)", WARMUP_ITERS, BENCH_ITERS, || {
                    SearchBuilder::default()
                        .location("/")
                        .ext("rs")
                        .build()
